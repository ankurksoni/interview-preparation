# MongoDB Interview Questions (Basic to Advanced – Product-Based Focused)
...
### 37. How does vector search work in MongoDB 7.0+?
**Answer:** MongoDB 7.0 introduced native vector indexing (beta), allowing ANN (approximate nearest neighbor) searches for use cases like semantic search, embeddings.

### 38. What is queryable encryption in MongoDB?
**Answer:** A feature allowing encrypted fields to be queried using equality searches without exposing plain data. Still in preview in 7.0.
...
### 51. How do you perform a text search in MongoDB?

**Answer:** MongoDB supports full-text search using a special text index. You must create this index on the fields you want to search and then use the `$text` operator in a query.
...
### 63. What are vector indexes and how are they used in MongoDB for AI?

**Answer:** MongoDB 7.0+ supports vector indexing for similarity search (e.g., embeddings). Use `vector` as index type.

```sh
db.images.createIndex({ embedding: "vector" }, { dimensions: 512 })
db.images.find({ embedding: { $near: [0.2, 0.3, 0.1, ...] } })
```
...
**Answer:** Check the `optimeDate` in `rs.printSlaveReplicationInfo()` and compare it to the primary.
...
### 85. What is vector data and how does MongoDB support it for AI/ML?

**Answer:** MongoDB 7.0+ supports vector search using the `vectorSearch` index.

```sh
db.items.createIndex({ embedding: "vector" }, { dimensions: 512 })
db.items.find({ embedding: { $near: [0.2, 0.3, ..., 0.9] } })
```

Used for similarity search on ML embeddings.
...
# MongoDB Advanced Interview Questions with Shell Examples (51–100)

... \[previous content unchanged up to 65] ...
